...
const Dentists = () => {
  ...
  const handleDelete = async (userId: number) => {
    await fetch(API_URL + "/users/dentists/" + userId, {
      method: "DELETE",
      headers: {
          ...
          <TableBody>
            {data.map((item, index) => (
              <TableRow key={index}>
                ...
                <TableCell>{(!item.specialization || item.specialization == '') ? 'N/A' : item.specialization}</TableCell>
                <TableCell>{(!item.qualification || item.qualification == '') ? 'N/A' : item.qualification}</TableCell>
